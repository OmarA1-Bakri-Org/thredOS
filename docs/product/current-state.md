...
- Step execution is wired into a real runtime flow. `app/api/run/route.ts` reads the sequence, compiles prompts, dispatches the runner, saves artifacts, persists deterministic gate decisions, and updates thread-surface state.
- Thread-surface persistence is real. `lib/thread-surfaces/repository.ts` persists the compatibility projection under `.threados/state/thread-surfaces.json` and also materializes canonical per-surface runtime records under `.threados/surfaces/<surfaceId>/surface.json`, `access.json`, `barrier.json`, and `state.json`.
...
- Gate workflow is materially implemented. `GateSchema` now includes `description`, `acceptance_conditions`, and `required_review`; `app/api/gate/route.ts` supports `insert`, `update`, `approve`, `block`, and `rm`; `lib/ui/api.ts` exposes `useUpdateGate`; `app/api/gate-decisions/route.ts` serves persisted deterministic decisions; and `components/workbench/sections/GateSection.tsx` renders condition-aware approval plus deterministic decision state.
...
- First-class pack installation exists. `app/api/packs/install/route.ts` compiles pack manifests into the active runtime, `lib/packs/install.ts` materializes sequence + prompts + thread surfaces transactionally, and `components/workbench/sections/PacksSection.tsx` exposes a manual install form.
...
- Run export is real and third-party-eval-ready in substance. `app/api/exports/run-bundle/route.ts` writes `.threados/exports/<runId>/bundle.json` and copies runtime trace / approval / gate-decision NDJSON files alongside it.
...
- Agent, builder, and pack context are partially integrated. `app/api/agent-profile/route.ts` combines agent registrations, pack state, runner state, and thread-surface state into a profile, but the broader builder/pack progression loop is not productized.
...
- **real today:** thredOS workbench, local API, CLI, sequence authoring, runtime surfaces, pack install, deterministic gate decisions, approvals, audit/policy foundations, export bundles, and basic agent registration/assignment
